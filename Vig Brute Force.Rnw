...
BStoreKey <- matrix(rep(0,30),ncol=3)
...
# run 2000 iterations of the Metropolis-Hastings algorithm
  for (iteration in 1:2000) {
    ...
    oldKey<-curKey
    newthing<-sample(1:26,v)
    for(m in 1:v){
      curKey[m]<-indexToChar(newthing[m])
    }
    curKey<-paste(curKey,collapse="")
  ...
  BStoreKey[v,1] <- curKey
  BStoreKey[v,2] <- oldScore
  BStoreKey[v,3] <- dur
  print(c(BStoreKey[v,1],BStoreKey[v,2],BStoreKey[v,3]))
  if (v==10){
  print(c(tolower(vigen(codemess,BStoreKey[which.max(BStoreKey[,2]),1],decrypt=T))))
  print(paste("The keyword is",BStoreKey[which.max(BStoreKey[,2]),1],"!"))
  }
}
library(ggplot2)
plot(BStoreKey[,3],type="l")
@
